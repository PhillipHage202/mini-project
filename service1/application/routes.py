...
def index():
    #gets a wep
    wep = requests.get("http://35.230.129.205:5001/wep")
    #gets element
    element = requests.get("http://35.230.129.205:5002/element")
    #gets character name
    character = str(wep.text) + "," + str(element.text)
    name = requests.post("http://35.230.129.205:5003/name", data=character)
